...
parser.add_argument('--testBatchSize', type=int, default=16, help='test batch size')
...
parser.add_argument('--shrink', type=float, default=0.23, help='scale perturbation ')
...
parser.add_argument('--targetClassifier', type=str, default='densenet121', help=" classifier (incv3 or vgg19 or vgg16)")
parser.add_argument('--dataset', type=str, default='ImageNet', help='the type of dataset')
parser.add_argument('--imagenetTrain', type=str, default='E:\\dataset\\ImageNet\\val\\val_sub', help='the path of dataset')
...
parser.add_argument('--netAttacker', default='C:\\Users\\user\\Desktop\\UAP_TOOL\\Attacks\\advres_imagenet_densenet121\\netAttacker_57_0.98833.t7', help="the path of netAttacker")
...
parser.add_argument('--outf', default='./advres_imagenet_densenet121_test', help='the path of netAttacker')
...
if opt.targetClassifier == 'incv3':
    ...
    netClassifier = torchvision.models.vgg19(pretrained=True)
    ...
    with torch.no_grad():
        for batch_idx, (inputv, cls) in enumerate(test_loader):
            ...
            progress_bar(batch_idx, len(test_loader),
                         "Val E%s, A_Succ %.5f success_count %.2f total_count %.2f L_inf %.5f L2 %.5f (Pert %.2f, Adv %.2f, Clean %.2f) C %.6f Skipped %.1f%%" % (
                         epoch, success_count / total_count,success_count,total_count, np.mean(L_inf), np.mean(dist), np.mean(pert_norm),
