...
###############################################################################
#  Reference: Generative Adversarial Perturbations (GAP)
...
parser.add_argument('--workers', type=int, help='workers', default=0)
...
parser.add_argument('--MaxIter', type=int, default=7500, help='Iterations in each Epoch')
parser.add_argument('--MaxIterTest', type=int, default=7500, help='Iterations in each Epoch')
parser.add_argument('--mag_in', type=float, default=10.0, help='l_inf magnitude of perturbation')
parser.add_argument('--expname', type=str, default='advgap_DenseNet', help='experiment name, output folder')
parser.add_argument('--checkpoint', type=str, default='', help='path to starting checkpoint')
parser.add_argument('--foolmodel', type=str, default='test', help='model to fool: "incv3", "vgg16", or "vgg19"')
...
parser.add_argument('--perturbation_type', default='universal', type=str, help='"universal" or "imdep" (image dependent)')
...
if opt.dataset == 'cifar10':
    ...
    training_data_loader = torch.utils.data.DataLoader(trainset, batch_size=opt.batchSize, shuffle=True, num_workers=opt.workers)
    testset = dset.CIFAR10(root=dataroot, train=False, download=True, transform=transform_test)
    testing_data_loader = torch.utils.data.DataLoader(testset, batch_size=opt.testBatchSize, shuffle=False, num_workers=opt.workers)
...
if opt.dataset == 'cifar10':
    ...
    pretrained_clf.load_state_dict(checkpoint['net'])
...
if not opt.explicit_U:
    ...
    netG = define_G(3, 3, 64, 'resnet_6blocks',
